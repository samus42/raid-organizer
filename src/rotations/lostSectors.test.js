...
describe('lost sector schedule', () => {
    it('starting day', () => {
        expect(getLostSector('2022-02-22')).toEqual({ name: 'Veles Labyrinth', area: Areas.Cosmodrome, reward: Rewards.Chest })
    })

    it('same sector as starting day, different reward', () => {
        expect(getLostSector('2022-03-05')).toEqual({ name: 'Veles Labyrinth', area: Areas.Cosmodrome, reward: Rewards.Arms })
    })

    it('last in rotation', () => {
        expect(getLostSector('2022-03-04')).toEqual({ name: 'Extraction', area: Areas.ThroneWorld, reward: Rewards.Legs })
    })
